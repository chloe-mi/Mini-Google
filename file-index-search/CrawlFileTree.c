...
bool CrawlFileTree(char *rootdir, DocTable **doctable, MemIndex **index) {
  ...
  // Try to open the directcalory using opendir().  If we fail, (e.g., we don't
...
static void HandleDir(char *dirpath, DIR *d, DocTable **doctable,
  ...
  for (i = 0 ; (dirent = readdir(d)) != NULL; i++) {
    ...
    if (stat(entries[i].path_name, &st) == 0) {
      ...
      // st_mode: file status's file type and mode (attributes)
  ...
  if (contents != NULL) {
    ...
    if (tab != NULL) {
      ...
      while (HTIterator_IsValid(it)) {
        ...
        // STEP 6.
        // Use HTIterator_Remove() to extract the next WordPositions structure
        // out of the hashtable. Then, use MemIndex_AddPostingList() to add the
        // word, document ID, and positions linked list into the inverted
        // index.
