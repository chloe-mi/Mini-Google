...
static const int kMaxPathSize = 1024;
...
bool IsPathSafe(const string &root_dir, const string &test_file) {
  ...
  // STEP 1
  char abs_dir_arg[kMaxPathSize];
  char abs_file_arg[kMaxPathSize];
  if (realpath(root_dir.c_str(), abs_dir_arg) == nullptr ||
      realpath(test_file.c_str(), abs_file_arg) == nullptr) {
    // error
    return false;
  }
  string abs_dir(abs_dir_arg);
  string abs_file(abs_file_arg);

  // realpath resolves /
  // make sure abs_dir ends in /
  // so if abs_dir is safe, abs_dir + unsafe chars doesn't also pass
  if (abs_dir.back() != '/') abs_dir += '/';

  return abs_file.compare(0, abs_dir.length(), abs_dir) == 0;
}
