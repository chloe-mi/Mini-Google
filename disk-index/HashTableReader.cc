...
HashTableReader::LookupElementPositions(HTKey_t hash_key) const {
  ...
  for (int i = 0; i < bucket_rec.chain_num_elements; i++) {
    IndexFileOffset_t elem_pos;
    Verify333(fread(&elem_pos, sizeof(IndexFileOffset_t), 1, file_) == 1);
    elem_pos = ntohl(elem_pos);
    ret_val.push_back(elem_pos);
  }
