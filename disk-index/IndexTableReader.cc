...
DocIDTableReader* IndexTableReader::LookupWord(const string &word) const {
  ...
  for (IndexFileOffset_t &offset : elements) {
    ...
    Verify333(fread(&header, sizeof(WordPostingsHeader), 1, file_)
              == 1);
    header.ToHostFormat();
    ...
    for (int i = 0; i < header.word_bytes; i++) {
      // STEP 2.
      char c;
      Verify333(fseek(file_, offset + sizeof(WordPostingsHeader) + i, SEEK_SET)
                == 0);
      Verify333(fread(&c, sizeof(char), 1, file_)
                == 1);
      ss << c;
    }
