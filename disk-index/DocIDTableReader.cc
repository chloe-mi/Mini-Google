...
bool DocIDTableReader::LookupDocID(
  ...
  for (IndexFileOffset_t &curr : elements) {
    ...
    Verify333(fread(&curr_header, sizeof(DocIDElementHeader), 1, file_)
              == 1);
    ...
    if (curr_header.doc_id == doc_id) {
      ...
      for (int i = 0; i < curr_header.num_positions; i++) {
        ...
        Verify333(fseek(file_, curr + sizeof(DocIDElementHeader)
                                    + i*sizeof(DocPositionOffset_t), SEEK_SET)
                  == 0);
        Verify333(fread(&pos, sizeof(DocPositionOffset_t), 1, file_)
                  == 1);
        pos = ntohl(pos);
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() const {
  ...
  for (int i = 0; i < header_.num_buckets; i++) {
    ...
    // the index file.
    IndexFileOffset_t bucket_rec_offset = offset_ + sizeof(BucketListHeader)
    ...
    for (int j = 0; j < bucket_rec.chain_num_elements; j++) {
      ...
      Verify333(fseek(file_, bucket_rec.position
                             + j*sizeof(ElementPositionRecord), SEEK_SET)
                == 0);
