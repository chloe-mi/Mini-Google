...
FileIndexReader::FileIndexReader(const string &file_name,
  ...
  #define FILEINDEXREADER_MAGIC_NUMBER 0xCAFEF00D
  Verify333(header_.magic_number == FILEINDEXREADER_MAGIC_NUMBER);
  #undef FILEINDEXREADER_MAGIC_NUMBER
  ...
  if (validate) {
    ...
    while (left_to_read > 0) {
      ...
      // read
      int count = (left_to_read > kBufSize) ? kBufSize : left_to_read;
      int success_count = static_cast<int>(fread(buf, sizeof(char),
                                                 count, file_));
      Verify333(success_count == count);

      // feed to CRC
      for (int i = 0; i < success_count; i++) {
        crc_obj.FoldByteIntoCRC(buf[i]);
      }
      left_to_read -= success_count;
    }
